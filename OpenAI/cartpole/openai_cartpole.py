...
# # Random input game generator
# def initialize_random_game():
...
def nn_model(input_size):
	...
	network = dropout(network, 0.8)  # keep_rate
...
def train_nn_model(training_data, model = False):
	X = np.array([i[0] for i in training_data]).reshape(-1, len(training_data[0][0]), 1)
	# print("X size", len(X))
	# print("Input size", len(X[0]))
	# print("reshaped by",len(training_data[0][0]))
...
print("Input size", len(training_data[0][0]))
model = train_nn_model(training_data)
# model.save("./model_cartpole/model_cartpole_nn.model")
